...
These steps cover how to set up a new robot in RobotStudio to work with the ABB driver. If using the Pack and Go, skip to the [controller confiugration step](#controller-configuration) for network configuration, and then to step 6 to start the controller.
...
These steps cover how to set up a new robot with an example external axis in RobotStudio to work with the ABB driver. If using the Pack and Go, skip to the [Controller Configuration step](#controller-configuration-multimove) for network configuration, and then to step 6 to start the controller.

## Simulation Setup (MultiMove)

1. Create a new solution similarly to the first step in the [Setting up a new robot](#setting-up-a-new-robot), enabling `604-2 MultiMove Independent` under Motion Coordination as well. You can also add MultiMove to an existing solution under the Controller option in the Ribbon, and select `Change Options`. Note that this will require a reset and restart of the controller, which will wipe the configuration and RAPID programs.
...
4. Navigate to and open the external axis - the external axis used in the example Pack and Go is the `MU_250__01` in the `Positioners` folder. You should see the new mechaism appear in the View window.
...
7. To attach the arm to the external axis, right click the arm name in the tree menu in the left pane, select Attach to --> MU_250__01. A pop up winodw will ask you to confirm if you want to update the position of the arm. Select Yes. A second pop up will ask if the robot should be coordinate with the track. Select Yes. A third prompt will ask you to restart the controller. Select Yes again.
...
16. On the left tree menu, expand the `RAPID` item. You should see two tasks that match the task names defined above.
    ...
    - In the module, change line 70 to reflect the mechanical unit name and the tranmission protocol name if not the same as in the example.
